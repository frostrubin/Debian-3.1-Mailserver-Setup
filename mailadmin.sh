...
{ 
  ...
  "Weiterleitung auf andere eMail-Adresse  einrichten" Lšschen	\
  "Account lšschen" 2> $FILE || exit
} 
...
show_list()
{
  ...
  # gewŸnschten Accountnamen in der Datei choice.$$
  # speichern
  eval   "whiptail                                 \
...
get_nick()
{
  whiptail --inputbox "Bitte das KŸrzel fŸr den zu erstellenden Account eingeben.  Die Mailadresse ergibt sich aus [kŸrzel]@schule.de" 12 60 2> $FILE
}
...
{
  whiptail --inputbox \
  "Geben Sie die eMail-Adresse ein, an die eMails fŸr den User $1 weitergeleitet werden sollen:" 12 60 2> $FILE
}

get_ack()
{
   whiptail --yesno "Sind Sie sicher, dass fŸr $NAME die eMail-Adresse $NICK@schule.de eingerichtet werden soll?" 12 60
}
...
do
  ...
  Erstellen)
    ...
    NAME=$(get_choice | sed 's/[^a-zA-Z0-9äöüß._ ]//g')
    ...
    echo "Bitte setzen Sie nun das Passwort fŸr den Mailaccount: "
    passwd $NICK 
  ...
  Lšschen)
    ...
    whiptail --defaultno --yesno "Wollen Sie die Mailbox des Users $NICK wirklich lšschen? Dabei werden eMails und sonstige persšnliche Dateien _unwiderruflich_ zerstšrt.\nAbfrage 1 von 3" 12 60 || continue
